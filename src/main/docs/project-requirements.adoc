...
// All text is British English and ISO-8859-1 only, per AGENT.md section 2.
// -------------------------------------------------------------------
...
using YAML Wire in <= 0.25 micros mean and <= 0.5 micros 99.9 %tile on 3.2 GHz x86-64. |JMH `SampleTime` benchmark, >= 10 M ops.
|CB-NF-P-002 |`Bytes.equalsBytes()` or equivalent region comparison method must be *>= 3 x* faster than
...
<= 20 micros 99.99 %tile. |JMH `SingleShot` benchmark, >= 1 000 repeats.
...
|CB-NF-P-008 |Resizing an elastic off-heap buffer (e.g., doubling capacity from 1MB to 2MB) should complete in <= 5 micros 99%tile, assuming underlying memory allocation is fast. |JMH benchmark measuring resize operation latency.
...
|CB-DOC-001 |All AsciiDoc and Javadoc *must* use British English and ISO-8859-1 (or UTF-8 where essential, clearly marked). |Automated spell- and byte-range scan in Maven `verify`.
...
ISO-8859-1 ::  American Standard Code for Information Interchange, 7-bit.
