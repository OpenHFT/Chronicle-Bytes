...
[cite: 116, 125] It considers system architecture details like endianness for performance.
[cite: 116]
`VanillaBytesStoreHash` ::
A general-purpose hashing algorithm for `BytesStore` instances.
[cite: 133] It provides a consistent hashing approach suitable for various data sizes.
`XxHash` ::
...
They should not be used for security-sensitive applications where collision resistance against malicious attacks is required.
[cite: 136]
