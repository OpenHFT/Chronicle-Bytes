...
Implementations exist for on-heap (`byte[]`), native off-heap (direct memory), and memory-mapped file regions xref:project-requirements.adoc#CB-FN-002[CB-FN-002].
* `Bytes<U>`: A mutable view over a `BytesStore` with independent read and write cursors (positions) and limits.
It provides a rich API for reading and writing primitive types, strings, and other data structures xref:project-requirements.adoc#CB-FN-001[CB-FN-001].
...
Always ensure off-heap `Bytes` instances are released to prevent memory leaks xref:project-requirements.adoc#CB-NF-O-001[CB-NF-O-001].
...
.Elastic Off-Heap Buffer xref:project-requirements.adoc#CB-FN-003[CB-FN-003]
[source,java]
----
...
Chronicle Bytes supports various string encodings and formats xref:project-requirements.adoc#CB-FN-009[CB-FN-009].
UTF-8 is common.
...
Inputs that exceed the remaining readable bytes trigger `BufferUnderflowException` xref:project-requirements.adoc#CB-NF-S-001[CB-NF-S-001].
...
xref:project-requirements.adoc#CB-FN-012[CB-FN-012] calls for correct handling.
...
Use try-with-resources diligently xref:project-requirements.adoc#CB-NF-O-003[CB-NF-O-003].
