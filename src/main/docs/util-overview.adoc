...
Every class is designed with Chronicle's core goals in mind - aiming for _low-allocation, low-latency, and direct-memory-friendly_ operations where appropriate.
...
Compression Streams ::
The `InputStream` and `OutputStream` wrappers returned by `Compression` strategies (like `LZW` or `GZIP`) are typically stateful and _not_ thread-safe.
...
These are stateful and _not_ thread-safe.
