...
public interface RandomDataInput extends RandomCommon {
     ...
     * @throws ThreadingIllegalStateException If this resource was accessed by multiple threads in an unsafe way
     */
    // CSCallerCheckedBounds delegate to caller because copyTo(byte[]) clamps len via Math.min(bytes.length, readRemaining()) so indices stay within the supplied array.
