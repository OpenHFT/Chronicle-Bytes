...
class MappedBytesEdgeTest extends BytesTestCommon {
    ...
    @MethodSource("data")
    @DisplayName("chunk resolution handles boundary read and write")
    public void testCorrectChunkResolved(int size, ReadWrite rw, String name, Consumer<Bytes<?>> doit) throws IOException {
        assumeFalse(Jvm.maxDirectMemory() == 0,
                "Direct memory must be available for mapped bytes edge test");
        ...
        try (final MappedBytes bytes = MappedBytes.mappedBytes(tempMBFile, CHUNK_SIZE, overlap)) {
            ...
            assertEquals(0, bytes.bytesStore().start(),
                    "Initial bytes store start is zero for " + name);

            if (rw == ReadWrite.WRITE) {
                checkWritePosition(bytes, CHUNK_SIZE + overlap - size, CHUNK_SIZE, doit);
                checkWritePosition(bytes, CHUNK_SIZE + overlap, CHUNK_SIZE, doit);
                checkWritePosition(bytes, CHUNK_SIZE + overlap + size, CHUNK_SIZE, doit);
                // go back to just before the overlap - will still be in second chunk
                checkWritePosition(bytes, CHUNK_SIZE + overlap - size, CHUNK_SIZE, doit);
                checkWritePosition(bytes, CHUNK_SIZE - size, 0, doit);
                // now try and write over the end of the chunk
                checkWritePosition(bytes, CHUNK_SIZE - 1, 0, doit);
                // and end of chunk plus offset
                checkWritePosition(bytes, CHUNK_SIZE + overlap - 1,  CHUNK_SIZE, doit);
                ...
                if (size > 1) {
                    ...
                    checkWritePosition(bytes, (3 * CHUNK_SIZE) - 1, 2 * CHUNK_SIZE, doit);
                ...
                checkReadPosition(bytes, CHUNK_SIZE - size, 0, doit);
                checkReadPosition(bytes, CHUNK_SIZE, 0, doit);
                checkReadPosition(bytes, CHUNK_SIZE + size, 0, doit);
                checkReadPosition(bytes, CHUNK_SIZE - size, 0, doit);

                if (size > 1) {
                    // read over the end should work as we have the overlap
                    checkReadPosition(bytes, CHUNK_SIZE - 1, 0, doit);

                    // we've got the first chunk loaded, now read from just before the end of the third chunk
                    checkReadPosition(bytes, (3 * CHUNK_SIZE) - 1, CHUNK_SIZE * 2, doit);
    ...
    private void checkWritePosition(MappedBytes bytes, long writePosition, long expectedStart, Consumer<Bytes<?>> doit) {
        bytes.writePosition(writePosition);
        doit.accept(bytes);
        assertEquals(expectedStart, bytes.bytesStore().start(),
                "Bytes store start matches expected after write at " + writePosition);
    }

    private void checkReadPosition(MappedBytes bytes, long readPosition, long expectedStart, Consumer<Bytes<?>> doit) {
        bytes.readPosition(readPosition);
        doit.accept(bytes);
        assertEquals(expectedStart, bytes.bytesStore().start(),
                "Bytes store start matches expected after read at " + readPosition);
    }
