...
class ByteStringAppenderTest extends BytesTestCommon {
    ...
    static Stream<Arguments> data() {
        return Stream.of(
//                Arguments.of("heap", false),
                Arguments.of("native", true)
        );
    }

    private Bytes<?> createBytes(boolean direct) {
        return direct ? Bytes.allocateElasticDirect() : Bytes.elasticByteBuffer();
    }
    ...
    void testConvertTo(String name, boolean direct) {
        ...
        try {
            ...
            VanillaBytes<Void> vb = Bytes.allocateElasticDirect(2);
            Bytes<?> one = ObjectUtils.convertTo(Bytes.class, 1);
            assertTrue(vb.append(1).contentEquals(one));
            one.releaseLast();
            hello1.releaseLast();
            hello.releaseLast();
            vb.releaseLast();
...
//            System.out.println(bytes);
    ...
    void testAppendDouble(String name, boolean direct)
        ...
        try {
            ...
            testAppendDouble0(bytes, -1.42278619425894E11);
        } finally {
            bytes.releaseLast();
        }
    }

    private void testAppendDouble0(Bytes<?> bytes, double d)
            throws IORuntimeException {
        bytes.clear();
