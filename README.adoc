...
For detailed requirements and design rationale, see link:src/main/docs/project-requirements.adoc[project-requirements.adoc] and link:src/main/docs/decision-log.adoc[decision-log.adoc].
...
image::src/main/docs/images/Figure1.png[450,450]
...
Obviously this is inefficient, however with appropriate caches in your lambda function, you can process and produce results incrementally.

=== Data in and out.

We model a Lambda function as having an interface for inputs and another for outputs.
...
Once we have interfaces, DTOs, and an implementation we can set up a test harness
...
`Bytes` instances provided a number of `toString` methods depending on the encoding of the data within the buffer.
As `Bytes` supports encoding of string data in both ISO-8859-1 and UTF-8, you will need to pick an appropriate `toString` method based on the encoding of the data within the buffer.
...
image::src/main/docs/images/customize-data-views-menu.jpg[]

image::src/main/docs/images/customize-data-views.png[]
