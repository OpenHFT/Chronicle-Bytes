...
Using bytesForRead() or bytesForWrite() methods, you can create Bytes from a section of a BytesStore
or a Bytes . The size of the new Bytes depends on `Limit` and `Position` cursors of the original Object.

- Create bytesForRead/Write From BytesStore:

. When you use bytesForWrite() `readLimit` and `writePosition` cursors are set to `start`.

. When you use bytesForRead(), `readPosition` is set to `start`, and `readLimit` and `writePosition` cursors
are set to `realCapacity`.

The new Bytes is not elastic and whether you use bytesForRead() or bytesForWrite() you can read and write from/into
the new Bytes using cursors.
...
. When you use bytesForWrite() `writeLimit` is set to `capacity` besides, the new Bytes is elastic.

. When you use bytesForRead() `readPosition` is set to `start`, and `writeLimit` is set to `realCapacity`
besides, the new Bytes is not elastic.

Whether you use bytesForRead() or byteForWrite(), You can both read and write from/into the new Bytes
using cursors.
[source,Java]
----
...
Standard Java ByteBuffer needs to be flipped to switch between reading and writing.
