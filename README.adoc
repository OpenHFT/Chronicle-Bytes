...
|===
| operation	| Indexed or streaming	| binary	| text | description | 
...
| CAS	| indexed	| int, long	| https://en.wikipedia.org/wiki/Compare-and-swap |
| volatile read/write	| indexed	| byte, short, int, long, float, double	| http://tutorials.jenkov.com/java-concurrency/volatile.html 	|
| peek	| both	| unsigned byte	| |
| stop bit compression	| streaming	| int,long, double, float, char	| see section below |
| search	| from start	| indexOf string, findByte	| https://stackoverflow.com/questions/5022956/space-efficient-long-representation	|
| addAndGet	| indexed	| float, double, int, long	| Atomically adds the given value to the current value. |
| copy	| from start	| write, copy	| 	|
| hash	| from start	| byteSum, fastHash	| https://en.wikipedia.org/wiki/Hash_function 	|
