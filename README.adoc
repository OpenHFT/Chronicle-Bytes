...
for a Bytes buffer.
- realCapacity: The current capacity of the buffer. Bytes are resizable when it is required.
...
[source,Java]
----
//Prints out buffer properties of bs
...
- In contrast to a BytesStore, a Bytes extends if you write into it data which is larger than the realCapacity of
a Bytes.
...
start 0
----
- Bytes gives you access to the cursors, thus you can use them to raed/write from/into a
...
//Figure 2 illustrates multiple threads accessing data in a
//BytesStore.
//
//[#img-Thread]
//.Multiple threads accessing a BytesStore.
//image::thread.jpg[300,300]

//=== bytesForRead() and bytesForWrite()
//you can create Bytes from a section of a BytesStore or a Bytes using bytesForRead() or
//bytesForWrite() methods. The new Bytes is not elastic and its size depends on `Limit` and `Position`
//cursors.
